...
stock LoadPlayerVehicleObj(playerid)
{
	new query[128];
	mysql_format(g_SQL, query, sizeof(query), "SELECT * FROM `vehicle_object` WHERE `owner` = %d", pData[playerid][pID]);
	...
	if(count > 0)
	{
		for(new z = 0; z < count; z++)
		{
			...
			cache_get_value_name_int(z, "toyid0", pvData[i][pvToyID][0]);
			cache_get_value_name_float(z, "toyposx0", pvData[i][pvToyPosX][0]);
			cache_get_value_name_float(z, "toyposy0", pvData[i][pvToyPosY][0]);
			cache_get_value_name_float(z, "toyposz0", pvData[i][pvToyPosZ][0]);
			// Root Position 0
			cache_get_value_name_float(z, "toyrotx0", pvData[i][pvToyRotX][0]);
			cache_get_value_name_float(z, "toyroty0", pvData[i][pvToyRotY][0]);
			cache_get_value_name_float(z, "toyrotz0", pvData[i][pvToyRotZ][0]);


			cache_get_value_name_int(z, "toyid1", pvData[i][pvToyID][1]);
			cache_get_value_name_float(z, "toyposx1", pvData[i][pvToyPosX][1]);
			cache_get_value_name_float(z, "toyposy1", pvData[i][pvToyPosY][1]);
			cache_get_value_name_float(z, "toyposz1", pvData[i][pvToyPosZ][1]);
			// Root Position 1
			cache_get_value_name_float(z, "toyrotx1", pvData[i][pvToyRotX][1]);
			cache_get_value_name_float(z, "toyroty1", pvData[i][pvToyRotY][1]);
			cache_get_value_name_float(z, "toyrotz1", pvData[i][pvToyRotZ][1]);

			cache_get_value_name_int(z, "toyid2", pvData[i][pvToyID][2]);
			cache_get_value_name_float(z, "toyposx2", pvData[i][pvToyPosX][2]);
			cache_get_value_name_float(z, "toyposy2", pvData[i][pvToyPosY][2]);
			cache_get_value_name_float(z, "toyposz2", pvData[i][pvToyPosZ][2]);
			// Root Position 2
			cache_get_value_name_float(z, "toyrotx2", pvData[i][pvToyRotX][2]);
			cache_get_value_name_float(z, "toyroty2", pvData[i][pvToyRotY][2]);
			cache_get_value_name_float(z, "toyrotz2", pvData[i][pvToyRotZ][2]);

			Iter_Add(PVehicles, i);
        }
		printf("[BASE_VEHOBJECT] Loaded player vehicle object from: %s(%d)", pData[playerid][pName], playerid);
	}
	return 1;
}
...
function LoadPlayerVehicle(playerid)
{
	...
	if(count > 0)
	{
		for(new z = 0; z < count; z++)
		{
			...
			cache_get_value_name_int(z, "death", pvData[i][cDeath]);
			Iter_Add(PVehicles, i);
			if(pvData[i][cClaim] == 0 && pvData[i][cDeath] == 0 && pvData[i][cPark] < 0)
			{
...
stock RemovePlayerVehicleObj(playerid)
{
	foreach(new i : PVehicles)
	{
		if(pvData[i][cOwner] == pData[playerid][pID])
		{
			...
			mysql_format(mysql, cQuery, sizeof(cQuery), "UPDATE `vehicle_object` SET ");
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyid0`=%d,", cQuery, pvData[i][pvToyID][0]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyid1`=%d,", cQuery, pvData[i][pvToyID][1]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyid2`=%d,", cQuery, pvData[i][pvToyID][2]);

			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyposx0`=%f,", cQuery, pvData[i][pvToyPosX][0]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyposx1`=%f,", cQuery, pvData[i][pvToyPosX][1]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyposx2`=%f,", cQuery, pvData[i][pvToyPosX][2]);

			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyposy0`=%f,", cQuery, pvData[i][pvToyPosY][0]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyposy1`=%f,", cQuery, pvData[i][pvToyPosY][1]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyposy2`=%f,", cQuery, pvData[i][pvToyPosY][2]);

			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyposz0`=%f,", cQuery, pvData[i][pvToyPosZ][0]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyposz1`=%f,", cQuery, pvData[i][pvToyPosZ][1]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyposz2`=%f,", cQuery, pvData[i][pvToyPosZ][2]);

			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyrotx0`=%f,", cQuery, pvData[i][pvToyRotX][0]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyrotx1`=%f,", cQuery, pvData[i][pvToyRotX][1]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyrotx2`=%f,", cQuery, pvData[i][pvToyRotX][2]);

			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyroty0`=%f,", cQuery, pvData[i][pvToyRotY][0]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyroty1`=%f,", cQuery, pvData[i][pvToyRotY][1]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyroty2`=%f,", cQuery, pvData[i][pvToyRotY][2]);

			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyrotz0`=%f,", cQuery, pvData[i][pvToyRotZ][0]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyrotz1`=%f,", cQuery, pvData[i][pvToyRotZ][1]);
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`toyrotz2`=%f,", cQuery, pvData[i][pvToyRotZ][2]);
			...
			{
			    DestroyDynamicObject(pvData[i][pvToy][idx]);
			}
...
function RemovePlayerVehicle(playerid)
{
	foreach(new i : PVehicles)
	{
		if(pvData[i][cOwner] == pData[playerid][pID])
		{
			...
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`death`=%d,", cQuery, pvData[i][cDeath]);
...
{
	ShowPlayerDialog(playerid, DIALOG_VEHCO, DIALOG_STYLE_LIST, "Vehicle Control","Engine\nLock\nLight\nStorage","Enter","Cancel");
}
    	...
    	if((carid = Vehicle_Nearest(playerid)) != -1)
    	{
    		if(Vehicle_IsOwner(playerid, carid))
    		{
    			if(!pvData[carid][cLocked])
    			{
    				...
    				GameTextForPlayer(playerid, "~w~VEHICLE ~r~LOCKED", 1000, 4);
    				PlayerPlaySound(playerid, 1145, 0.0, 0.0, 0.0);
    			...
    			else
    			{
    				...
    				GameTextForPlayer(playerid, "~w~VEHICLE ~g~UNLOCKED", 1000, 4);
    				PlayerPlaySound(playerid, 1145, 0.0, 0.0, 0.0);
