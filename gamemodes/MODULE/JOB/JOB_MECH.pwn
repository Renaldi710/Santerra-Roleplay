...
IsAtMech(playerid)
{
	...
	{
		if(IsPlayerInRangeOfPoint(playerid, 15.0, 2185.061279, -2205.058837, 13.884775) || IsPlayerInRangeOfPoint(playerid, 15.0, 2191.608154, -2198.419189, 13.884775) ||
			IsPlayerInRangeOfPoint(playerid, 15.0, 2197.721923, -2192.280517, 13.884775) || IsPlayerInRangeOfPoint(playerid, 15.0, 2216.275634, -2227.101562, 13.780775) ||
			IsPlayerInRangeOfPoint(playerid, 30, 2210.014892, -2233.172119, 13.788775) ||
			IsPlayerInRangeOfPoint(playerid, 15.0, 2203.341308, -2239.390380, 13.804775))
		{
			return 1;
		}
...
function EngineFix(playerid, vehicleid)
{
	...
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
		if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
		{
			if(pData[playerid][pActivityTime] >= 100 && IsValidVehicle(vehicleid))
			{
				SetValidVehicleHealth(vehicleid, 100);
				//new panels, doors, light, tires;
				//GetVehicleDamageStatus(vehicleid, panels, doors, light, tires);
				//UpdateVehicleDamageStatus(vehicleid, 0, 0, 0, tires);
