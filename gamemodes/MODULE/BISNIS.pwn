...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		{
			format(string, sizeof(string), "{FFFFFF}[ID:%d]\n{FFFFFF}Store: {FFFFFF}%s\n{FFFFFF}Type: {FFFFFF}%s\n{FFFFFF}Owner: {FFFFFF}%s\n{FFFFFF}Location {FFFFFF}%s", id, bData[id][bName], type, bData[id][bOwner], GetLocation(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]));
			bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 8.0);
        	bData[id][pickupBcp] = CreateDynamicCP(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 2.0, 0, 0, -1, 2.0);
        }
        else
        {
            format(string, sizeof(string), "{FFFFFF}[B:%d]\n{FFFFFF}Price: {FFFFFF}%s\n{FFFFFF}Type: {FFFFFF}%s\n{FFFFFF}Location: {FFFFFF}%s", id, FormatMoney(bData[id][bPrice]), type,  GetLocation(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]));
            bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 8.0);
        	bData[id][pickupBcp] = CreateDynamicCP(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 2.0, 0, 0, -1, 2.0);
        }
