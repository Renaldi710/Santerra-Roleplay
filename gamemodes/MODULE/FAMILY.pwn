...
CMD:gotofam(playerid, params[])
{
	...
	if(sscanf(params, "d", id))
		return Usage(playerid, "/gotofam [id]");
	if(!Iter_Contains(FAMILYS, id)) return Error(playerid, "Family ID tidak ada.");

	SetPlayerPosition(playerid, fData[id][fExtposX], fData[id][fExtposY], fData[id][fExtposZ], 2.0);
    SetPlayerInterior(playerid, fData[id][fInt]);
	Servers(playerid, "Teleport ke ID Family %d", id);
	return 1;
}
