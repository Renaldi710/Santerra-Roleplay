...
new Float:VNudgeVal[MAX_PLAYERS];
...
CMD:editven(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, VENTOYSET_VALUE, DIALOG_STYLE_INPUT, "Vending Toy PosX", "Set current float value\nNormal Value = 0.05\n\nEnter Float NudgeValue in here:", "Select", "Back");
	return 1;
}
	...
	if(dialogid == VTOYSET_VALUE)
	{
		if(response)
		{
			if(isnull(inputtext))
			{
				...
				ShowPlayerDialog(playerid, VENSELECT_POS, DIALOG_STYLE_LIST, "Select Editing Pos", "Position X\nPosition Y\nPosition Z\nPosition RX\nPosition RY\nPosition RZ", "Select", "Back");
			}
			else
			{
...
/* =======================[ Timer goes here ]======================= */
function AddVenObjPos(playerid)
{
	new vendingid = Iter_Free(Vending), gametext[36];
	if(pData[playerid][EditStatus] == VFloatX)
	{
		VendingData[vendingid][VendingPosX] += VNudgeVal[playerid];
		format(gametext, 36, "Float X: ~w~%f", VendingData[vendingid][VendingPosX]);
	}
	else if(pData[playerid][EditStatus] == VFloatY)
	{
		VendingData[vendingid][VendingPosY] += VNudgeVal[playerid];
		format(gametext, 36, "Float Y: ~w~%f", VendingData[vendingid][VendingPosY]);
	}
	else if(pData[playerid][EditStatus] == VFloatZ)
	{
		VendingData[vendingid][VendingPosZ] += VNudgeVal[playerid];
		format(gametext, 36, "Float Z: ~w~%f", VendingData[vendingid][VendingPosZ]);
	}
	else if(pData[playerid][EditStatus] == VFloatRX)
	{
		VendingData[vendingid][VendingPosRX] += VNudgeVal[playerid];
		format(gametext, 36, "Float RX: ~w~%f", VendingData[vendingid][VendingPosRX]);
	}
	else if(pData[playerid][EditStatus] == VFloatRY)
	{
		VendingData[vendingid][VendingPosRY] += VNudgeVal[playerid];
		format(gametext, 36, "Float RY: ~w~%f", VendingData[vendingid][VendingPosRY]);
	}
	else if(pData[playerid][EditStatus] == VFloatRZ)
	{
		VendingData[vendingid][VendingPosRZ] += VNudgeVal[playerid];
		format(gametext, 36, "Float RZ: ~w~%f", VendingData[vendingid][VendingPosRZ]);
	}
	PlayerTextDrawSetString(playerid, EditVObjTD[playerid][5], gametext);
}
function SubVenObjPos(playerid)
{
	new vendingid = Iter_Free(Vending), gametext[36];
	if(pData[playerid][EditStatus] == VFloatX)
	{
