	...
	if(rows)
  	{
		...
		while(i < rows)
		{
			...
			ppData[id][parkPickup] = CreateDynamicPickup(1239, 23, ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ], ppData[id][parkWorld],  ppData[id][parkInt]);
			ppData[id][pickupCpp] = CreateDynamicCP(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ], 2.0, 0, 0, -1, 2.0);
			ppData[id][parkLabel] = CreateDynamic3DTextLabel(str, COLOR_LBLUE, ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]+0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, ppData[id][parkWorld], ppData[id][parkInt], -1, 10.0);
			Iter_Add(Parks, id);
	    	i++;
		}
...
CMD:setgaragepos(playerid, params[])
{
	...
	ppData[id][parkY] = y;
	ppData[id][parkZ] = z;
	ppData[id][parkInt] = GetPlayerInterior(playerid); //pickupCpp
	ppData[id][parkWorld] = GetPlayerVirtualWorld(playerid);
	
	DestroyDynamicCP(ppData[id][pickupCpp]);
	DestroyDynamicPickup(ppData[id][pickupCpp]);
