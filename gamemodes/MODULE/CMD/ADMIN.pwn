CMD:acmds(playerid)
{
	...
	new line3[2480];
	strcat(line3, ""WHITE_E"Administrator Commands:"AQUA"\n\
	...
	strcat(line3, "\n\n"WHITE_E"Senior Admin Commands:"AQUA"\n\
	...
	strcat(line3, "\n\n"WHITE_E"Lead Admin Commands:"AQUA"\n\
	/oban /banip /unbanip /reloadweap /resetweap /sethbe\n\
 	/createdoor /editdoor");

	strcat(line3, ""WHITE_E"\n\nHead Admin Commands:"AQUA"\n\
	/setname /setvip /setfaction /setleader /takemoney /takegold /giveweap\n\
	/veh /destroyveh");

	strcat(line3, "\n\n"WHITE_E"Server Owner Commands:"AQUA"\n\
	/sethelperlevel /setcs /setadminname /setmoney /givemoney /setbankmoney /givebankmoney\n\
	/setmaterial /setcomponent /createpv /destroypv /explode\n\n\
	"WHITE_E"Developer:"AQUA"\n\
	...
	strcat(line3, "\n"AQUA"LC:RP "WHITE_E"- Anti-Cheat is actived.\n\
...
CMD:hcmds(playerid)
{
	...
	new line3[2480];
	strcat(line3, ""WHITE_E"Junior Helper Commands:"AQUA"\n\
	...
	strcat(line3, "\n\n"WHITE_E"Head Helper Commands:"AQUA"\n\
	/respawnsapd /respawnsags /respawnsamd /respawnsana /respawnjobs\n");
 	
	strcat(line3, "\n"AQUA"LP:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"SoI:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:setcs(playerid, params[])
{
	...
	new otherid;
	if(sscanf(params, "u", otherid)) return Usage(playerid, "/setcs <ID/Name>");
	if(pData[otherid][pPlayerCS] != 0) return Error(playerid, "Player tersebut sudah memiliki CS!");
	pData[otherid][pPlayerCS] = 1;
	Info(playerid, "Anda telah menset character story player %s", pData[otherid][pName]);
	Info(otherid, "Admin %s telah memberikan character story kepada kamu!", pData[playerid][pAdminname]);
...
CMD:aduty(playerid, params[])
{
	...
	if(!pData[playerid][pAdminDuty])
    {
		if(pData[playerid][pAdmin] > 0)
		{
			...
			SendStaffMessage(COLOR_LIGHTRED, "AdmCmd: %s is now on duty as administrator.", GetStaffRank(playerid), pData[playerid][pName]);
		}
		else
		{
...
CMD:ban(playerid, params[])
{
	...
	if(datez != 0)
	{
		...
		SendClientMessageToAllEx(COLOR_BAN, "Reason: %s ", tmp);
	 	format(string, sizeof string, "```\nBanned: %s Has Been Banned Player %s. REASON: %s\n```",  pData[playerid][pAdminname], giveplayer, datez, tmp);
	...
	else
	{
		...
		SendClientMessageToAllEx(COLOR_BAN, "Reason: %s ", tmp);
		format(string, sizeof string, "```\nBanned: %s Has Been Permanently Banned %s. REASON: %s\n```",  pData[playerid][pAdminname], giveplayer, tmp);
		DCC_SendChannelMessage(g_discord_ban, string);
	}
	//SetPlayerPosition(otherid, 227.46, 110.0, 999.02, 360.0000, 10);
...
CMD:respawnsapd(playerid, params[])
{
	...
	{
		if(IsVehicleEmpty(SAPDVehicles))
		{
			SetVehicleToRespawn(SAPDVehicles);
			SetValidVehicleHealth(SAPDVehicles, 2000);
			SetVehicleFuel(SAPDVehicles, 1000);
			SwitchVehicleDoors(SAPDVehicles, false);
		}
	}
...
CMD:respawnsamd(playerid, params[])
{
	...
	{
		if(IsVehicleEmpty(SAMDVehicles))
		{
			SetVehicleToRespawn(SAMDVehicles);
			SetValidVehicleHealth(SAMDVehicles, 2000);
			SetVehicleFuel(SAMDVehicles, 1000);
			SwitchVehicleDoors(SAMDVehicles, false);
		}
	}
...
CMD:oban(playerid, params[])
{
	...
	new query[128], string[1200], giveplayer[1200];
	format(string, sizeof string, "```\nBanned: %s Has Been Banned Offline Player %s. REASON: %s\n```",  pData[playerid][pAdminname], PlayerName, giveplayer);
	DCC_SendChannelMessage(g_discord_ban, string);
...
CMD:givecoint(playerid, params[])
{
	if(pData[playerid][pAdmin] < 5)
		return PermissionError(playerid);

	new coint, otherid, tmp[64];
	if(sscanf(params, "ud", otherid, coint))
	{
	    Usage(playerid, "/givecoint <ID/Name> <legcoint>");
	    return true;
	}
	...
	GivePlayerCointEx(otherid, coint);

	Coint(playerid, "Kamu telah mengset leg coing %s(%d) menjadi %s!", pData[otherid][pName], otherid, FormatCoint(coint));
	Coint(otherid, "Admin %s telah mengset legcoint anda menjadi %s!",pData[playerid][pAdminname], FormatCoint(coint));

	format(tmp, sizeof(tmp), "%d", coint);
	StaffCommandLog("SETCOINT", playerid, otherid, tmp);
	return 1;
}
...
CMD:giveallmoney(playerid, params[])
{
	...
	extract params -> new money; else return Usage(playerid, "/giveallmoney <ammount>");
...
CMD:tpm(playerid, params[])
{
    ...
    {
        SetVehiclePos(vehicleid, GetPVarFloat(playerid, "pointx"), GetPVarFloat(playerid, "pointy"), GetPVarFloat(playerid, "pointz+10"));
    }
    else
