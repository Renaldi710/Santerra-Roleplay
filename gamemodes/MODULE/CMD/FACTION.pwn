...
CMD:pdpv(playerid)
{
    ...
    if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1599.85,-1708.79,6.19063)) return Error(playerid, "Anda harus berada di garasi sapd!");
	ShowPlayerDialog(playerid, DIALOG_PVSAPD, DIALOG_STYLE_LIST, "SAPD VEHICLE", "Police LS\nFire truck\nFire Truck\nPolice Ranger\nS.W.A.T\nFbi Truck\nSultan\nHPV1000\nTow Truck\nPolice Maveric\nInfernus", "Ambil", "Cancel");
    return 1;
}

CMD:mdpv(playerid, params[])
{
	if(pData[playerid][pFaction] != 3)
	   return Error(playerid, "You dont have Faction Samd!");
    if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1132.24, -1334.37, 14.2844)) return Error(playerid, "Anda harus berada di garasi samd!");
	ShowPlayerDialog(playerid, DIALOG_PVSAMD, DIALOG_STYLE_LIST, "SAMD VEHICLE", "Fire Truck\nRaindance[HELICOPTERS]\nFire Truck\nAmbulance\nWayfer\nPremier\nRomero\nMaveric[HELICOPERS]", "Ambil", "Cancel");
    return 1;
}
CMD:adespawn(playerid, params[])
{
