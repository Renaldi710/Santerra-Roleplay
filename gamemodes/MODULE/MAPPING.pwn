...
Mapping_Refresh(id)
{
    if(id != -1)
    {
		...
		mpData[id][mLabel] = CreateDynamic3DTextLabel(string, 0xFFFF82FF, mpData[id][mPosX], mpData[id][mPosY], mpData[id][mPosZ], 20);
...
CMD:editmp(playerid, params[])
{
			...
			if(!strcmp(type, "pos", true))
		    {
		        ...
		        if(sscanf(string, "s[24]f", types, amount))
		            return Usage(playerid, "/editmp [id] [pos] [pos x-y-z] [amount]");
