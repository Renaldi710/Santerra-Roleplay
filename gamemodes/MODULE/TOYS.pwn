...
CMD:toys(playerid)
{
	...
	if(pToys[playerid][0][toy_model] == 0)
	{
	    strcat(string, ""dot"Bug 1\n");
	}
	else strcat(string, ""dot"tolol bug ajg "LG_E"(Used)\n");

	if(pToys[playerid][1][toy_model] == 0)
	{
	    strcat(string, ""dot"Slot 2\n");
	}
	else strcat(string, ""dot"Slot 2 ""(Used)\n");

	if(pToys[playerid][2][toy_model] == 0)
	{
	    strcat(string, ""dot"Slot 3\n");
	}
	else strcat(string, ""dot"Slot 3 ""(Used)\n");

	if(pToys[playerid][3][toy_model] == 0)
	{
	    strcat(string, ""dot"Slot 4\n");
	}
	else strcat(string, ""dot"Slot 4 ""(Used)\n");

	strcat(string, ""dot">""Reset Toys");

	ShowPlayerDialog(playerid, DIALOG_TOY, DIALOG_STYLE_LIST, ""RED_E"Clarnce: "WHITE_E"Player Toys", string, "Select", "Cancel");
	return 1;
}
