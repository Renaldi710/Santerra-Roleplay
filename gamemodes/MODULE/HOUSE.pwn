...
House_Refresh(houseid)
{
    if(houseid != -1)
    {
		...
		if(IsValidDynamicCP(hData[houseid][hCP]))
  		...
  		if(IsValidDynamicCP(hData[houseid][pickupCp]))
            DestroyDynamicCP(hData[houseid][pickupCp]);
		...
		{
			format(string, sizeof(string), "{00FFFF}[H:%d]\n{FFFF00}Owner: {FFFFFF}%s\n{BABABA}%s", houseid, hData[houseid][hOwner], GetLocation(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]));
			hData[houseid][hPickup] = CreateDynamicPickup(19130, 23, hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]+0.2, 0, 0, _, 10.0);
			hData[houseid][pickupCp] = CreateDynamicCP(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ], 2.0, 0, 0, -1, 2.0);
        }
        else
        {
            format(string, sizeof(string), "{00FFFF}[H:%d]\n{FFFF00}Type: {FFFFFF}%s\n{FFFF00}Price: {FFFFFF}%s\n{BABABA}%s", houseid, type, FormatMoney(hData[houseid][hPrice]), GetLocation(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]));
            hData[houseid][hPickup] = CreateDynamicPickup(19130, 23, hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]+0.2, 0, 0, _, 10.0);
			hData[houseid][pickupCp] = CreateDynamicCP(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ], 2.0, 0, 0, -1, 2.0);
		}
...
CMD:myhouse(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_HOUSES, DIALOG_STYLE_LIST, "{FF0000}LegacyPride: {0000FF}Houses", CMDSString, "Select", "Cancel");
	return 1;
}
