...
CreateArmsPoint()
{
	//JOBS
	new strings[128];
	CreateDynamicPickup(1239, 23, -2816.98, -1528.64, 141.144, -1);
	format(strings, sizeof(strings), "[Black Market]\n{FFFFFF}/creategun");
	CreateDynamic3DTextLabel(strings, COLOR_GREY, -2816.98, -1528.64, 141.144, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); //
}

CMD:creategun(playerid, params[])
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.5,  -2816.98, -1528.64, 141.144)) return Error(playerid, "You must in Black Market!");
	if(pData[playerid][pFamily] != -1)
	{
		new Dstring[512];
		format(Dstring, sizeof(Dstring), "Weapon\tMats & Comps & Price\n\
		Silenced Pistol(ammo 70)\t150 & 200 / $5.000.00\n");
		...
		format(Dstring, sizeof(Dstring), "%sAK-47(ammo 100)\t500 & 500 & $25.000.00\n", Dstring);
        format(Dstring, sizeof(Dstring), "%sKatana(×1)\t200 & 200 & $10.000.00\n", Dstring);
        format(Dstring, sizeof(Dstring), "%sArmor(×1)\t120 & 120 & $5.000.00\n", Dstring);
