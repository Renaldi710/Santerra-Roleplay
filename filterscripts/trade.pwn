...
public CreditsEzpz()
{
    ...
    printf(" ======================================================== ");
    printf("|							 |");
    printf("|    Trade System By Zenn Loaded..   |");
    printf("|							 |");
    printf(" ======================================================== ");
    printf("");
}
...
CMD:trade(playerid, params[])
{
	...
	{
	    piData[playerid][moneyput] = money;
		piData[playerid][put] = pointData[MAX_POINTS - 1][val];
		GivePlayerMoney(playerid, -money);
		piData[playerid][buy] = 1; // buy
		format(mstr, 256, "You have put on buy at %f", piData[playerid][put]);
	...
	else if(!strcmp(bbuy, "sell", true, 4))
	{
		...
		piData[playerid][put] = pointData[MAX_POINTS - 1][val];
		GivePlayerMoney(playerid, -money);
		piData[playerid][buy] = 2; // sell
		format(mstr, 256, "You have put on sell at %f", piData[playerid][put]);
		SendClientMessage(playerid, -1, mstr);
	}
...
{
	if(piData[playerid][moneyput] == 0) return SendClientMessage(playerid, -1, "ERROR");
	new Float:diff;
	if(piData[playerid][buy] == 1) diff = pointData[MAX_POINTS-1][val] - piData[playerid][put];
	else if(piData[playerid][buy] == 2) diff = piData[playerid][put] - pointData[MAX_POINTS - 1][val];
	new multi = floatround(diff);
	GivePlayerMoney(playerid, multi*piData[playerid][moneyput]);
	new mstr[256];
	format(mstr, 256, "You have earned %i.", multi*piData[playerid][moneyput]);
	SendClientMessage(playerid, -1, mstr);
	piData[playerid][moneyput] = 0;
	piData[playerid][put] = 0;
	piData[playerid][buy] = 0;
	return 1;
}

CMD:tradechart(playerid, params[])
{
	if(piData[playerid][showing] == 0)
	{
	    GRAPHIC::ShowForPlayer(playerid, MY_GRAPH);
	    TextDrawShowForPlayer(playerid, TDEditor_PTD);
		piData[playerid][showing] = 1;
	}
	else
	{
	    GRAPHIC::HideForPlayer(playerid, MY_GRAPH);
	    TextDrawHideForPlayer(playerid, TDEditor_PTD);
		piData[playerid][showing] = 0;
	}
